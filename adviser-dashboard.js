...
var currentAdviser = window.currentAdviser || null;
// null means we haven't attempted to load assignments yet or they failed; an
// empty array means we successfully queried and found none.
var teachingAssignments = (typeof window.teachingAssignments !== 'undefined') ? window.teachingAssignments : null;
var allStudents = window.allStudents || [];

// --- BEGIN: tenantFetch implementation ---
function resolveSchoolCode() {
    // ensure school query param exists (defaulting to derived or default-school)
    try {
...
async function tenantFetch(pathOrUrl, options = {}) {
    ...
    // make fetch cancellable so that a hung request doesn't block a dashboard
    const controller = new AbortController();
    const signal = controller.signal;
    // default timeout of 10 seconds unless caller overrides
...
async function fetchDashboardData() {
    // Get current adviser from session storage first.  If the stored user has a
    // different role (admin, student, etc.) we do *not* treat them as an adviser;
    // instead we'll let initializeDashboard notice and redirect later.
    try {
        const userDataRaw = sessionStorage.getItem('adviserData') || sessionStorage.getItem('teacherData') || localStorage.getItem('loggedInUser');
        if (userDataRaw) {
            const userData = JSON.parse(userDataRaw);
            if (userData && userData.id) {
                // some logins (admin) also set loggedInUser; guard by role
                const role = String(userData.role || '').toLowerCase();
                if (!role || role.includes('adviser') || role.includes('teacher')) {
                    currentAdviser = userData; // stored user may still be called teacherData for compatibility
                    window.currentAdviser = currentAdviser;
                    console.log('[Dashboard] Using adviser from storage:', currentAdviser);
                } else {
                    console.log('[Dashboard] Stored user is not an adviser (role=' + role + '), ignoring', userData);
                }
            }
        }
    } catch (e) {
        console.warn('[Dashboard] Error reading adviser data from storage:', e);
    }

    // If no adviser in storage, try to fetch from profile API
    if (!currentAdviser) {
        try {
            const profileResp = await tenantFetch('/api/adviser-auth/profile');
            if (profileResp.ok) {
                const profileData = await profileResp.json();
                if (profileData.adviser) {
                    currentAdviser = profileData.adviser;
                    window.currentAdviser = currentAdviser;
                    console.log('[Dashboard] Fetched adviser from profile API:', currentAdviser);
                }
            }
        } catch (e) {
            console.warn('[Dashboard] Adviser profile API failed:', e);
        }
    }

    // Fetch teaching assignments if we have an adviser
    if (currentAdviser && currentAdviser.id) {
        if (!Array.isArray(teachingAssignments) || teachingAssignments.length === 0) {
            try {
                const assignResp = await tenantFetch(`/api/adviser-auth/teaching-assignments/${currentAdviser.id}`);
                if (assignResp.ok) {
                    ...
                    // server responded but not OK: mark failure explicitly
                    teachingAssignments = null;
                    console.warn('[Dashboard] Teaching assignments request returned', assignResp.status);
                }
            } catch (e) {
    ...
    if (!Array.isArray(allStudents) || allStudents.length === 0) {
        try {
            ...
            if (studentsResp.ok) {
                ...
                // refresh cards now that students list is available
                try { await populateDashboardCards(); } catch(_e) { }
            }
        } catch (e) {
...
async function initializeDashboard() {
    console.log('[Dashboard] initializeDashboard running');
    try {
        // apply school-specific branding before anything else
        try {
            await bootstrapSchoolBranding();
        } catch (_e) {
            console.warn('[Dashboard] bootstrapSchoolBranding failed', _e);
        }
        // first populate currentAdviser and related caches from storage/profile
        await fetchDashboardData();

        // if login data is missing, redirect to the proper dashboard instead of
        // always forcing the user back to the generic login page.
        if (!currentAdviser || !currentAdviser.id) {
            // detect alternate sessions and forward accordingly
            ...
            // a teacher might be logged in (role=teacher) but not an adviser
            if (sessionStorage.getItem('teacherData') || localStorage.getItem('loggedInUser')) {
                window.location.href = withSchoolParam('teacher-dashboard.html');
                return;
            }

            showDebugInfo('No logged-in adviser detected; redirecting to login');
            setTimeout(() => {
                window.location.href = withSchoolParam('auth.html?role=adviser');
            }, 1000);
            return;
        }

        // Setup UI interactions (header, sidebar, etc.)
        ...
        window.addEventListener('hashchange', () => {
            ...
            // directly switch sections rather than dispatching another click
            switchToSection(sectionFromHash);
        });

        // Load data with debug output.  Run potentially long fetches in parallel and
        // do not block the UI.  We'll populate the summary cards immediately and
        // then refresh them again once the data has finished loading (or failed).

        const assignmentsPromise = loadTeachingAssignments().catch(e => {
            showDebugInfo('Teaching assignments failed to load.');
            console.error('[Dashboard] Teaching assignments error:', e);
        });
        const studentsPromise = loadAllStudents().catch(e => {
            showDebugInfo('All students failed to load.');
            console.error('[Dashboard] Students load error:', e);
        });
        let sectionsPromise = Promise.resolve();
        const id = currentAdviser?.id || null;
        if (id) {
            sectionsPromise = loadAssignedSections(id).then(() => {
                console.log('[Dashboard] Assigned sections loaded');
            }).catch(e => {
                console.error('[Dashboard] loadAssignedSections error during init:', e);
            });
        }

        // populate cards right away so the "Loading..." text goes away even if the
        // above requests are still in flight. the cards will be refreshed again
        // below after all promises settle.
        try {
        ...
        // wait for all data loads to finish (with the timeout built into tenantFetch)
        await Promise.all([assignmentsPromise, studentsPromise, sectionsPromise]);
        // refresh cards after data has arrived
        try {
            await populateDashboardCards();
        } catch (e) {
            showDebugInfo('Dashboard cards failed to refresh.');
            console.error('[Dashboard] Card refresh error:', e);
        }
        await refreshAdviserNotificationBadge(false);
        setInterval(() => { refreshAdviserNotificationBadge(false).catch(() => {}); }, 15000);
        startRolePollingForAdviserDashboard();
    } catch (error) {
        // show a debug panel but do not automatically log the user out – the error
        // might be minor (e.g. notification badge failure) and should not kick the
        ...
        if (!debugPanel) {
            ...
            const sectionList = document.querySelector('#adviser-dashboard .section-list');
            if (sectionList) sectionList.appendChild(debugPanel);
        }
        debugPanel.textContent = 'Dashboard error: ' + (error?.message || error);
        console.error('[Dashboard] initialization caught error, user remains on page', error);
        // we intentionally do not redirect to auth here
    }
}

// Entry point
function startAdviserDashboard() {
    initializeDashboard().catch(err => {
        console.error('[Dashboard] initializeDashboard threw', err);
        showDebugInfo('Initialization error: ' + (err?.message||err));
    });
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', startAdviserDashboard);
} else {
    // already past loading state
    startAdviserDashboard();
}
...
function normalizeHashSection(hash) {
    if (!hash || typeof hash !== 'string') return 'adviser-dashboard';
    const clean = hash.replace('#', '').trim();
    return clean || 'adviser-dashboard';
}

function showDebugInfo(message) {
    // display a visible debug panel on the dashboard and log to console
    let debugPanel = document.getElementById('dashboardDebugPanel');
    if (!debugPanel) {
        debugPanel = document.createElement('div');
        debugPanel.id = 'dashboardDebugPanel';
...
function applyTeacherInfoToDom() {
    try {
        ...
        // Update adviser name display
        const adviserNameDisplay = document.getElementById('adviserNameDisplay');
        if (adviserNameDisplay) {
            adviserNameDisplay.textContent = currentAdviser.name || 'Unknown Adviser';
        }

        // Update adviser role display
        const adviserRoleDisplay = document.getElementById('adviserRoleDisplay');
        if (adviserRoleDisplay) {
            adviserRoleDisplay.textContent = 'Adviser';
        }

        // Update profile name in header
        const adviserProfileName = document.getElementById('adviserProfileName');
        if (adviserProfileName) {
            adviserProfileName.textContent = currentAdviser.name || 'Adviser';
        }

        // Update settings fields if they exist
        const settingsName = document.getElementById('settingsName');
        if (settingsName) {
            settingsName.value = currentAdviser.name || '';
        }

        const settingsEmail = document.getElementById('settingsEmail');
        if (settingsEmail) {
            settingsEmail.value = currentAdviser.email || '';
        }

        const settingsEmployeeId = document.getElementById('settingsEmployeeId');
        if (settingsEmployeeId) {
            settingsEmployeeId.value = currentAdviser.teacher_id || currentAdviser.employee_id || '';
        }

        console.log('[Dashboard] ✅ Adviser info applied to DOM:', currentAdviser.name);
    } catch (error) {
...
async function markAdviserNotificationRead(notificationId) {
    const adviserId = getCurrentAdviserId();
    if (!adviserId) throw new Error('Adviser ID not found');
...
async function deleteAdviserNotification(notificationId) {
    const adviserId = getCurrentAdviserId();
    if (!adviserId) throw new Error('Adviser ID not found');
...
async function markAllAdviserNotificationsRead() {
    const adviserId = getCurrentAdviserId();
    if (!adviserId) throw new Error('Adviser ID not found');
    
    const resp = await tenantFetch(`/api/notifications/teacher/${adviserId}/read-all`, {
        method: 'POST'
    });
...
async function refreshAdviserNotificationBadge(showLoading = true) {
    ...
    try {
        ...
        const resp = await tenantFetch(`/api/notifications/teacher/${adviserId}/unread-count`);
        if (resp.ok) {
...
function resolveTeacherIdentifiers() {
    ...
    if (currentAdviser) {
        if (currentAdviser.id) identifiers.ids.push(String(currentAdviser.id));
        if (currentAdviser.email) identifiers.emails.push(String(currentAdviser.email));
        if (currentAdviser.employee_id) identifiers.ids.push(String(currentAdviser.employee_id));
    }
...
// Utility: programmatically switch visible section and update sidebar/URL
function switchToSection(sectionId) {
    if (!sectionId) return;
    // clear old active flags and reset display styles
    ...
    const menuItem = document.querySelector(`.sidebar-menu .menu-item[data-section="${sectionId}"]`);
    if (menuItem) {
    ...
    if (target) {
        ...
        // show section (let CSS handle block vs flex etc)
        target.style.display = '';
    }

    // update hash without causing scroll
    const newHash = sectionId === 'adviser-dashboard' ? '' : `#${sectionId}`;
...
function updateHashForSection(section) {
    const newHash = section === 'adviser-dashboard' ? '' : `#${section}`;
    if (window.location.hash !== newHash) {
...
async function loadTeacherNotifications() {
    ensureTeacherNotificationCenterUi();
    const teacherId = getcurrentAdviserId();
    const listEl = document.getElementById('teacherNotifList');
    ...
    if (readAllBtn && readAllBtn.dataset.bound !== '1') {
        ...
        readAllBtn.addEventListener('click', async () => {
            try {
                await markAllAdviserNotificationsRead();
                await loadTeacherNotifications();
    ...
    if (listEl && listEl.dataset.bound !== '1') {
        ...
        listEl.addEventListener('click', async (event) => {
            ...
            try {
                if (action === 'read') {
                    await markAdviserNotificationRead(notificationId);
                } else if (action === 'delete') {
                    await deleteAdviserNotification(notificationId);
                }
                await loadTeacherNotifications();
            } catch (err) {
...
function redirectByNormalizedRole(normalizedRole) {
    ...
    // if already on adviser dashboard, do not redirect anywhere
...
function startRolePollingForAdviserDashboard() {
    if (rolePollingHandle) clearInterval(rolePollingHandle);
    const userEmail = currentAdviser && currentAdviser.email;
    if (!userEmail) return;
    ...
    const poll = async () => {
        try {
            ...
            const currentRole = normalizeTeacherRoleValue(currentAdviser && currentAdviser.role);

            if (latestRole && latestRole !== currentRole) {
                currentAdviser.role = latestRole;
...
async function fetchFreshRole() {
    console.log('[Adviser Dashboard] === FETCHING FRESH ROLE FROM API ===');
    ...
    try {
        ...
        console.warn('[Adviser Dashboard] Could not get email from storage:', e.message);
    }
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (payload.teacher && payload.teacher.role) {
                    ...
                    console.log('[Adviser Dashboard] ✅ Fresh role from API:', role);
                    return role;
                }
            }
        } catch (e) {
            console.warn('[Adviser Dashboard] Error fetching fresh role:', e.message);
        }
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (role) {
                console.log('[Adviser Dashboard] ✅ Fresh role from profile endpoint:', role);
                return role;
            }
        }
    } catch (e) {
        console.warn('[Adviser Dashboard] Error with profile endpoint:', e.message);
    }
    
    console.warn('[Adviser Dashboard] ❌ Could not fetch fresh role from API');
    return null;
}

// legacy initialization removed – the dashboard now relies exclusively on initializeDashboard()
// which is wired up near the top of this script with a single DOMContentLoaded listener.
...
async function populateDashboardCards() {
    try {
        ...
        // Adviser name
        if (!currentAdviser || !currentAdviser.name) {
            adviserNameDisplay.textContent = 'No data found';
            errorMsg += 'Adviser name missing. ';
            // Force redirect to login if adviser data is missing/corrupted
            setTimeout(() => {
                window.location.href = withSchoolParam('auth.html?role=adviser');
            }, 1500);
        } else {
            adviserNameDisplay.textContent = currentAdviser.name;
        }
        document.getElementById('adviserRoleDisplay').textContent = 'Adviser';
        ...
        let subjectCount = 0;
        let studentCount = 0;
        // it's acceptable for an adviser to have zero subjects; only flag an error if we
        // genuinely failed to load the assignments (we'll use null for that case).
        if (teachingAssignments === null) {
            subjectCount = 'No data found';
            errorMsg += 'Teaching assignments could not be retrieved. ';
        } else if (Array.isArray(teachingAssignments) && teachingAssignments.length > 0) {
            ...
            // empty array means no subjects – show 0 but do not treat as an error
            subjectCount = 0;
        }
        if (Array.isArray(allStudents) && allStudents.length > 0) {
        ...
        if (adviserNameDisplay.textContent === 'No data found' || schoolYear === 'No data found' || subjectCount === 'No data found' || studentCount === 'No data found') {
            if (!errorPanel) {
            ...
            if (!retryBtn) {
                ...
                retryBtn.onclick = async () => {
                    adviserNameDisplay.textContent = 'Loading...';
        ...
        if (!retryBtn) {
            ...
            retryBtn.style = 'background: #f093fb; color: white; padding: 10px 15px; margin-top: 18px;';
            retryBtn.onclick = async () => {
                document.getElementById('adviserNameDisplay').textContent = 'Loading...';
...
/**
 * Load assigned sections for the adviser (advisory + teaching)
 * @param {string} adviserId
 */
async function loadAssignedSections(adviserId) {
    const advisoryContainer = document.getElementById('advisoryCard');
    if (advisoryContainer) {
        advisoryContainer.innerHTML = '<p class="loading">Loading advisory section...</p>';
    }
    ...
    try {
        ...
        if (adviserId) {
            // adviser-specific advisory assignments
            try {
                const url = `/api/adviser-auth/sections/${encodeURIComponent(adviserId)}`;
                const resp = await tenantFetch(url, { cache: 'no-store' });
                if (resp.ok) {
                    let data = await resp.json();
                    if (data && data.sections && Array.isArray(data.sections)) advSections = data.sections;
                    else if (Array.isArray(data)) advSections = data;
                } else {
                    console.warn('[Adviser Dashboard] adviser sections request failed:', resp.status);
                }
            } catch (e) {
                console.warn('[Adviser Dashboard] Exception fetching adviser sections:', e.message);
            }
            console.log('[Adviser Dashboard] advSections retrieved:', advSections.length, advSections);

            // also include any sections where adviser appears as teacher
            try {
                const url2 = `/api/adviser-dashboard/sections-teacher/${encodeURIComponent(adviserId)}`;
                const resp2 = await tenantFetch(url2, { cache: 'no-store' });
                if (resp2.ok) {
                    const raw = await resp2.text();
                    try {
                        let data = raw ? JSON.parse(raw) : null;
                        if (Array.isArray(data)) teacherSections = teacherSections.concat(data);
                        else if (data && data.sections && Array.isArray(data.sections)) teacherSections = teacherSections.concat(data.sections);
                        else if (data && data.data && Array.isArray(data.data)) teacherSections = teacherSections.concat(data.data);
                    } catch (parseErr) {
                        console.warn('[Adviser Dashboard] could not parse sections-teacher JSON:', parseErr, 'raw', raw);
                    }
                } else {
                    console.warn('[Adviser Dashboard] sections-teacher request failed:', resp2.status);
                }
            } catch (e) {
                console.warn('[Adviser Dashboard] Exception fetching sections-teacher:', e.message);
            }
            console.log('[Adviser Dashboard] teacherSections retrieved:', teacherSections.length, teacherSections);
        }

        // combine and filter active year
        let combined = [];
        if (advSections.length) combined = combined.concat(advSections);
        if (teacherSections.length) combined = combined.concat(teacherSections);
        const activeYear = Number(window.activeSchoolYearId || 0) || null;
        if (activeYear && combined.length) {
            combined = combined.filter(s => Number(s.school_year_id || s.schoolYearId || 0) === activeYear);
        }
        ...
        const assigned = [];
        combined.forEach(s => {
            const key = String(s.section_id || s.id || s.section_code || s.code || '').trim();
            if (key && !seen.has(key)) {
                seen.add(key);
                assigned.push(s);
            }
        });
        const assignedSections = assigned;

        if (advisoryContainer) {
            if (!assignedSections || assignedSections.length === 0) {
                advisoryContainer.innerHTML = '<p class="no-data">No advisory section assigned</p>';
            } else {
                advisoryContainer.innerHTML = assignedSections.map((s) => {
                    const sid = s.section_id || s.id || s.sectionId || '';
                    const scode = s.section_code || s.code || sid || '';
                    const sname = s.section_name || s.name || '';
                    const grade = s.grade || s.grade_level || s.level || '';
                    const isAdv = advSections.some(a => String(a.section_id||a.id||a.section_code||'') === String(sid));
                    const advClass = isAdv ? ' advisory-highlight' : '';
                    return `
                        <div class="advisory-assignment${advClass}" onclick="viewSubjectStudents('${escapeHtml(sid)}','Advisory','${escapeHtml(scode)}','${escapeHtml(sname)}')">
                            <div class="advisory-subject">${escapeHtml(scode)}</div>
                            <div class="advisory-meta">${escapeHtml(sname)} – Grade ${escapeHtml(grade)}</div>
                        </div>
                    `;
                }).join('');
            }
        ...
        console.error('[Adviser Dashboard] Error loading advisory sections:', err);
        if (advisoryContainer) advisoryContainer.innerHTML = '<p class="error-message">Error loading advisory section</p>';
    }

    // refresh teaching assignments too
    try {
        await loadTeachingAssignments();
    } catch (_e) {
        console.warn('[Adviser Dashboard] loadTeachingAssignments during assignedSections refresh failed');
    }
}
...
async function loadTeachingAssignments() {
    try {
        ...
        if (!container) {
            console.log('[Adviser Dashboard] Container not found, skipping');
            return;
        }

        container.innerHTML = '<p class="loading">Loading teaching assignments...</p>';
        console.log('[Adviser Dashboard] ===== LOADING ASSIGNMENTS =====');
        console.log('[Adviser Dashboard] Current adviser:', currentAdviser);
        console.log('[Adviser Dashboard] Adviser ID:', currentAdviser?.id);
        ...
        if (!endpoints.length) {
            ...
            teachingAssignments = null; // treat as failure
            return;
        }
        ...
        for (const url of endpoints) {
            console.log('[Adviser Dashboard] 🔄 Trying endpoint:', url);
            try {
                resp = await tenantFetch(url);
                console.log('[Adviser Dashboard] Response status:', resp.status, resp.statusText);
                
                if (resp.ok) {
                    usedUrl = url;
                    console.log('[Adviser Dashboard] ✅ SUCCESS with:', url);
                    break;
                } else {
                    console.warn('[Adviser Dashboard] ❌ Failed:', url, 'Status:', resp.status);
                }
            } catch (e) {
                console.warn('[Adviser Dashboard] ❌ Exception:', url, e.message);
                continue;
            }
        }

        if (!resp || !resp.ok) {
            console.warn('[Adviser Dashboard] ❌ All endpoints failed');
        ...
        try {
            ...
            console.log('[Adviser Dashboard] Content-Type:', contentType);
            
            if (contentType && contentType.includes('application/json')) {
                data = await resp.json();
                console.log('[Adviser Dashboard] ✅ Parsed JSON successfully');
            } else {
                console.warn('[Adviser Dashboard] ❌ Response is not JSON, got:', contentType);
                container.innerHTML = '<p class="no-data">No teaching assignments found</p>';
                teachingAssignments = null;
                return;
            }
        } catch (parseError) {
            console.warn('[Adviser Dashboard] ❌ Failed to parse JSON response:', parseError);
            container.innerHTML = '<p class="no-data">No teaching assignments found</p>';
            teachingAssignments = null;
            return;
        }
        ...
        console.log('[Adviser Dashboard] Raw API response:', data);
        
        let assignments = [];
        if (Array.isArray(data)) {
            console.log('[Adviser Dashboard] Response is direct array');
            assignments = data;
        } else if (data && data.assignments && Array.isArray(data.assignments)) {
            console.log('[Adviser Dashboard] Response has .assignments array');
            assignments = data.assignments;
        } else if (data && data.data && Array.isArray(data.data)) {
            console.log('[Adviser Dashboard] Response has .data array');
            assignments = data.data;
        } else if (data && typeof data === 'object') {
            console.log('[Adviser Dashboard] Response is object with keys:', Object.keys(data));
            assignments = [];
        }
        ...
        console.log('[Adviser Dashboard] ✅ Teaching assignments loaded:', teachingAssignments.length);
        // update summary cards in case they were rendered earlier with partial data
        try { await populateDashboardCards(); } catch(_e) { /* ignore */ }

        if (!teachingAssignments || teachingAssignments.length === 0) {
            console.warn('[Adviser Dashboard] No assignments in data');
            container.innerHTML = '<p class="no-data">No teaching assignments found</p>';
            return;
        }
        ...
        console.error('[Adviser Dashboard] Error loading assignments:', error);
        const container = document.getElementById('teachingAssignmentsCard');
        if (container) {
...
function viewSubjectStudents(sectionId, subjectName, sectionCode = '', sectionName = '') {
    console.log('[Adviser Dashboard] Viewing students for section:', sectionId, 'subject:', subjectName, 'sectionCode:', sectionCode);
...
function displayTeachingAssignmentStudents() {
    ...
    console.log('[Adviser Dashboard] Displaying', sectionStudents.length, 'students for section context:', currentSubjectViewing);
...
/**
 * Go back to My Sections view from teaching assignment student list
 */
function backToTeachingAssignments() {
    ...
    // Show my-sections section
    document.getElementById('my-sections').style.display = 'block';
    
    // Activate My Subjects menu item
    document.querySelectorAll('.sidebar .menu-item').forEach(mi => mi.classList.remove('active'));
    document.querySelector('[data-section="my-sections"]')?.classList.add('active');
    updateHashForSection('my-sections');
}
...
async function loadAllStudents() {
    try {
        console.log('[Adviser Dashboard] Loading all students');
        ...
        if (enrollments && enrollments.length > 0) {
            ...
            console.log('[Adviser Dashboard] Loaded', allStudents.length, 'students');
        }
        ...
        console.error('[Adviser Dashboard] Error loading students:', error);
...
function buildTeachingPrintContext(rows) {
    ...
    const teacherName = String(currentAdviser?.name || document.getElementById('adviserProfileName')?.textContent || 'Adviser').trim();
...
function setupHeaderInteractions() {
    ...
    // bind sidebar menu clicks once
    const sidebarMenu = document.querySelector('.sidebar-menu');
    if (sidebarMenu && sidebarMenu.dataset.bound !== '1') {
        sidebarMenu.dataset.bound = '1';
        sidebarMenu.addEventListener('click', (event) => {
            const item = event.target.closest('.menu-item[data-section]');
            if (!item) return;
            event.preventDefault();
...
function getcurrentAdviserId() {
    return Number(currentAdviser?.id || currentAdviser?.teacher_id || 0) || null;
}

async function refreshAdviserNotificationBadge(showMessage = false) {
    const badge = document.getElementById('notificationBadge');
    const teacherId = getcurrentAdviserId();
